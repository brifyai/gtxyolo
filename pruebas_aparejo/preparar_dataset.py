...
1. Filtro administrative: Si el reporte de fallas de la API se debe puramente a
...
# Cargamos YOLO-World solo si está disponible para evitar fallos de importación previous
try:
...
# Palabras clave de daño físico estructural (si están presentes, NO es administrative)
STRUCTURAL_KEYWORDS = [
    "corte",
    "desgaste",
    "oxid",
...
def es_defecto_administrativo(comentario):
    """Retorna True si el defecto es puramente administrative (sin daño estructural)."""
    if not comentario:
        return False
...
def recolectar():
    ...
    while True:
        ...
        for p in items:
            ...
            # Filtro Administrative: Si está dado de baja pero solo es administrative, re-clasificar como Conforme
        ...
        print(
            f"  Página {page} leída. Conforme: {len(buckets['Conforme'])} | Dado de baja: {len(buckets['Dado de baja'])}"
        )
        page += 1
    return buckets
...
def main():
    ...
    for estado, slug in CLASES.items():
        ...
        splits = [("train", muestra[:n_tr]), ("val", muestra[n_tr : n_tr + n_va]), ("test", muestra[n_tr + n_va :])]
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(descargar_imagen, reg, split, slug, detector): (reg, split, slug)
            for reg, split, slug in tareas
        }
